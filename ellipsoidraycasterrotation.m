...
kl=zeros(raycount,3); for v=1:pointlightscount; printf("v/plcount: %i/%i\n",v,pointlightscount); pla=(ones(kcicount,1)*pointlights(v,1:3)); minlightdist=1.1; #0.00000001 #1.01
plv=pla.-knc; pld=sqrt(sum(plv.^2,2)); pld3=pld*ones(1,3); plnv=plv./pld3; lv=[knc.+(minlightdist.*plnv) plnv]; [ck, ckn, ckuv] = raysphereintersection(scenespheres(:,1:7), lv);
cki=find(ck(:,2)>(nspheres-pointlightscount)); ckd=ck(cki,1); ckdr=ckd.^(-2); ckicount=size(cki,1); ckni=ckn(cki,:); kcni=kci(cki,:); pld3n=pld3(cki,:);
