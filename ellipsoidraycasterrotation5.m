...
lightvalue=1; pointlightscount = 5; pointlights = [100*(rand(pointlightscount,3)-0.5) 500*randi(255,pointlightscount,3)/255 -2*ones(pointlightscount,1)]; pointlightscount+=1; pointlights=[pointlights; [500 500 500 1 1 1 0]];
...
plv=pla.-knc; pld=sqrt(sum(plv.^2,2)); pld3=pld*ones(1,3); plnv=plv./pld3; lv=[knc.+(minlightdist.*plnv) plnv]; [ck, ckn] = raysphereintersection(scenespheres(:,1:7), lv);
cki=find(ck(:,2)==(nspheres-pointlightscount+v)); ckd=ck(cki,1); ckdr=ckd.^pllc; ckicount=size(cki,1); ckni=ckn(cki,:); kcni=kci(cki,:); pld3n=pld3(cki,:);
plc=ones(ckicount,1)*pointlights(v,4:6); kl(kcni,:).+=(lightvalue.*ckdr.*plc); figure(1); hold on; plot3(ckni(:,1),ckni(:,2),ckni(:,3),'r.'); hold off; endfor;
